<template>
  ...
  import { fetchPrayerForWeekday, getDefaultPageTitle, getTodayWeekdayKey } from "@/utils/dailyPrayer"
  ...
  async function load() {
    ...
    try {
      ...
      pageTitle.value = data.pageTitle || getDefaultPageTitle(todayKey)
      sections.value = data.sections || []
      openIndex.value = data.defaultOpenIndex >= 0 ? data.defaultOpenIndex : 0
    } catch (e) {
      pageTitle.value = getDefaultPageTitle(todayKey)
  ...
  /* ===== settings box copied/adapted from your component ===== */
  .mkSettingsBox{
    margin: 10px 0 12px;
    padding: 14px;
    border-radius: 20px;
