...
<template>
      ...
      <ion-tabs>
        ...
        <ion-tab-bar :class="['mkTabBar', themeClass]" slot="bottom">
          ...
          <ion-tab-button
            ...
            :class="{ mkSelected: activeTab === 'settings' }"
          >
            <ion-icon :icon="settingsOutline" />
            <ion-label>{{ isArabic ? 'الإعدادات' : 'Settings' }}</ion-label>
          </ion-tab-button>
  
          <!-- More (Noor moved inside) -->
          <ion-tab-button
            tab="more"
            class="mkTabBtn"
            href="#"
  ...
  // ✅ Settings modal via query
  function openSettings() {
    router.push({ path: route.path, query: { ...route.query, modal: 'settings' } })
  }
  ...
  const moreButtons = computed(() => {
    ...
    const openModal = (modal: 'about') => {
      router.push({ path: route.path, query: { ...route.query, modal } })
    }
  
    return [
      {
        text: isArabic.value ? 'نور العالم' : 'Noor Al Alam',
        icon: tvOutline,
        handler: () => go('/noor-alalam'),
      },
      {
